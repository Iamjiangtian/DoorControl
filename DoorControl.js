...
/* 
这是一段游戏脚本中控制门的开关的代码，选择展示这段代码是因为它有两个优点： 普适性和可再利用。
模型师提供的门的模型中，中心点在门的正中，但是门开关时，旋转所依据的轴并不是正中心的点，而是两旁的其中一条边（也就是日常生活中的荷叶hinge）。
所以我们需要修改模型的中心点来实现门的开关。但是这样做需要修改每个门模型的参数，工作量较大。
这段代码中用简单的数学方法计算实时的荷叶位置，减少了模型师的工作量，并且通用于所有门的开关，即插即用、易于维护，在将来的项目中也可以再重复使用。
